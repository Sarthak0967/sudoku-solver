...
def warped_image_saving():
    ...
    saved_once = False  # prevent saving every frame
    ...
    while True:
        ...
        if sudoku_detected:
            ...
            if sudoku_cnt is not None:
                ...
                # ✅ SAVE WARPED IMAGE ONCE
                if not saved_once:
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    save_path = f"results/current_warped.jpg"
                    cv2.imwrite(save_path, warped)
                    print(f"✔ Warped image saved at: {save_path}")
                    saved_once = True
